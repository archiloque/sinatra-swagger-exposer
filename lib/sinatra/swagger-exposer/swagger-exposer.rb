...
module Sinatra
  ...
  module SwaggerContentCreator

    def self.create_swagger_content(types, endpoints, info)
      result_endpoints = {}
    ...
    def self.create_swagger_type(type)
      result_type = {}
      ...
      [:required, :example].each do |property_key|
    ...
    KNOWN_SWAGGER_TYPE_PROPERTIES = [:type, :format, :description]

    def self.create_swagger_type_property(property_name, property_info)
      result = {}
      KNOWN_SWAGGER_TYPE_PROPERTIES.each do |known_property|
      ...
      property_info.each_pair do |property_name, property_value|
        unless KNOWN_SWAGGER_TYPE_PROPERTIES.include? property_name
          if logging?
            $stderr.puts "Swagger: unknown property [#{property_name}] for #{property_name}: #{property_info}"
          end
        end
      end
    ...
    def self.create_swagger_endpoint(endpoint)
      result_endpoint = {
      ...
      if responses && (!responses.empty?)
        ...
        responses.each_pair do |code, content|
          ...
          if content[:params] && content[:params][:type]
            ...
            result_response[:schema] = result_schema
          else
            result_schema = {
                '$ref' => "#/definitions/#{content[:type]}",
            }
  ...
  module SwaggerExposer
    ...
    def self.registered(app)
      ...
      app.get '/swagger_doc.json' do
        result_endpoints = SwaggerContentCreator.create_swagger_content(
            settings.swagger_types,
            settings.swagger_endpoints,
            settings.swagger_info
        )
        STDOUT << "#{result_endpoints}\n"
        content_type :json
